...
- Start virtual machines.
  Assuming existing disks =${QVM_DIR}/machines/{cp,wk1,wk2}.qcow2=:
#+begin_src sh :tangle
...
- The VM serial port and QEMU monitor will be multiplexed on a pty mentioned in the output of =qvm-run=.
  To get a console from the host:
#+begin_src
  foot --pty dev/pts/3
#+end_src


- Alternatively, SSH to the VMs from the host.
  Aassuming the SSH public key was authorized on the guest by cloud-init:
#+begin_src sh
